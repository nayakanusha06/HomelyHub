...
const FilterModal = ({selectedFilters,onFilterChange,onClose}) => {
  ...
  const [amenities,setAmenitiesType] =useState(selectedFilters.amenities || [])

  useEffect(() => {
    setPriceRange({
      min: selectedFilters.priceRange?.min || 600,
      max: selectedFilters.priceRange?.min || 30000,
  })
  setPropertyType(selectedFilters.propertyType || "")
  setRoomType(selectedFilters.roomType || "")
  setAmenitiesType(selectedFilters.amenities || [])
  },[selectedFilters])
  ...
  const handleAmenitiesChange = (selectAmenity) => {
    setAmenitiesType((prevAmenities) =>
      prevAmenities.includes(selectAmenity)
        ? prevAmenities.filter((item) => item !== selectAmenity)
        : [...prevAmenities, selectAmenity]
    );
  };
